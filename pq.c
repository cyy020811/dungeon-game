...
/* Check if the vertex is in the priority queue */
int elementOf(struct pq* pq, void* item) {
  for (int i = 0; i < pq->count; i++) {
    if ((int)(size_t)(pq->queue)[i] == (int)(size_t)item) {
      return 1;
    }
  }
  return 0;
...
void update(struct pq* pq, void* item, int dist) {
  int i = 0;
  while ((int)(size_t)(pq->queue)[i] != (int)(size_t)item) {
    i++;
    if (i > pq->count) return;
  }
  (pq->priorities)[i] = dist;
}
